...
int TcpServer::setListen(unsigned short port)
{
	...
	{
		ret = -1;
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func socket() err");
		return ret;
	}
	...
	{
		ret = -1;
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func setsockopt() err");
		return ret;
	}
	...
	{
		ret = -1;
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func bind() err");
		return ret;
	}
	...
	{
		ret = -1;
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func listen() err");
		return ret;
	}
...
TcpSocket* TcpServer::acceptConn(int timeout)
{
	...
	if (connfd < 0)
	{
		if (connfd == -1 && errno == ETIMEDOUT)
		{
			...
			m_log.Log(__FILE__, __LINE__, DDLog::ERROR, connfd, "func acceptConn() TimeOutError");
		}
		else
		{
			m_log.Log(__FILE__, __LINE__, DDLog::ERROR, connfd, "func acceptConn() OtherError");
		}
		return NULL;
	}
...
int TcpServer::acceptTimeout(int wait_seconds)
{
	...
	{
		ret = -1;
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func accept() err");
		return ret;
	}
