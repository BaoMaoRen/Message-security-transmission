...
int TcpSocket::connectToHost(const char * ip, unsigned short port, int timeout)
{
	...
	{
		ret = ParamError;
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func sckClient_connect() err, check  (ip==NULL || connfd==NULL || port<=0 || port>65537 || connecttime < 0)");
		return ret;
	}
	...
	if (ret < 0)
	{
		...
		else
		{
			...
			m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func connect_timeout() err");
		}
	}
...
int TcpSocket::sendMsg(const char * sendData, int dataLen, int timeout)
{
	...
	{
		ret = ParamError;
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func sckClient_send() err, check (data == NULL || datalen <= 0)");
		return ret;
	}
...
int TcpSocket::recvMsg(char ** recvData, int & recvLen, int timeout)
{
	...
	if (ret != 0)
	{
		...
		{
			ret = TimeoutError;
			m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func read_timeout() timeout");
			return ret;
		}
		else
		{
			m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func read_timeout() err");
			return ret;
		}
	}
	...
	if (ret == -1)
	{
		...
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func readn() err");
		return ret;
	}
	else if (ret < 4)
	{
		...
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func readn() err, peer closed");
		return ret;
	}
	...
	{
		ret = MallocError;
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "malloc() err");
		return ret;
	}
	...
	{
		//printf("func readn() err:%d \n", ret);
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "readn() err");
		return ret;
	}
	else if (ret < n)
	{
...
int TcpSocket::blockIO(int fd)
{
	...
	{
		ret = flags;
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func activate_nonblock() err");
		return ret;
	}
...
int TcpSocket::noBlockIO(int fd)
{
	...
	{
		ret = flags;
		m_log.Log(__FILE__, __LINE__, DDLog::ERROR, ret, "func deactivate_nonblock() err");
		return ret;
	}
